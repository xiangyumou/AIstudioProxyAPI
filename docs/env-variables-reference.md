...
- **类型**: 布尔值
- **默认值**: `false`
- **示例**: `MULTI_SESSION_ENABLED=true`
- **说明**: 启用后，系统将从 `auth_profiles/saved/` 目录加载多个认证文件，创建独立的浏览器会话池。请求将在多个会话间轮询分配，提升并发能力和容错性。

### SESSION_POOL_SIZE

- **用途**: 会话池大小限制
- **类型**: 整数
- **默认值**: `0`（无限制，使用目录中所有认证文件）
- **示例**: `SESSION_POOL_SIZE=3`
- **说明**: 限制最大会话数量。设为 0 时自动使用目录中所有可用的认证文件。
